...
import { AdminDeleteUserApi } from '@/api/admin/user';
import { message, Modal } from 'ant-design-vue';
import { AdminDeleteOrderApi } from '@/api/admin/order';
import { AdminDeleteDeliveryApi } from '@/api/admin/delivery';
import { AdminDeletePlatformAdminApi } from '@/api/admin/platformAdmin';
...
export function useRowSelection(
  ...
  function deleteSelectRowByKey() {
    const selectedRows = unref(selectedRowRef);
    ...
    Modal.confirm({
      ...
      content: '您确定要删除选中数据吗？',
      onOk() {
        if (selectedRows[0].defaultDeliveryAddress !== undefined) {
          // 用户
          const queryString = selectedRows.map((item) => `ids=${item.userID}`).join('&');
          AdminDeleteUserApi(queryString).then(() => {
            message.success('数据已删除，请刷新表格');
          });
        } else if (selectedRows[0].note !== undefined) {
          // 订单
          const queryString = selectedRows.map((item) => `ids=${item.orderID}`).join('&');
          AdminDeleteOrderApi(queryString).then(() => {
            message.success('数据已删除，请刷新表格');
          });
        } else if (selectedRows[0].deliveryPersonID !== undefined) {
          // 配送
          const queryString = selectedRows.map((item) => `ids=${item.deliveryPersonID}`).join('&');
          AdminDeleteDeliveryApi(queryString).then(() => {
            message.success('数据已删除，请刷新表格');
          });
        } else if (selectedRows[0].adminName !== undefined) {
          // 管理员
          const queryString = selectedRows.map((item) => `ids=${item.adminID}`).join('&');
          AdminDeletePlatformAdminApi(queryString).then(() => {
            message.success('数据已删除，请刷新表格');
          });
        } else {
          message.error('删除失败');
