<template>
  <PageWrapper :class="prefixCls" title="我接的历史订单">
...
<script lang="ts" setup>
  ...
  import { DeliveryGetMyOrdersApi } from '@/api/delivery/orders';
  ...
  const fetchOrders = async () => {
    const response = await DeliveryGetMyOrdersApi({ deliveryID: userID, orderID: null }); // 调用获取订单数据的接口
  ...
  const filterByStatus = (list: typeof orderList.value) => {
    return list.filter((item) => item.status === 5 || item.status === 6 || item.status === 7);
  };
